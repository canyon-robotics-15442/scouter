defmodule Scouter.Vex do
  ...
  def upsert_events(raw_events) do
    Enum.map(raw_events, fn raw ->
      ...
      |> Repo.insert!(on_conflict: {:replace, [:name, :date, :region, :updated_at]}, conflict_target: :vex_id)
    end)
  end
...
def upsert_event_teams(event, raw_teams) do
  Enum.each(raw_teams, fn raw ->
    team =
      ...
      |> Repo.insert!(on_conflict: {:replace, [:name, :number, :updated_at]}, conflict_target: :vex_id)
